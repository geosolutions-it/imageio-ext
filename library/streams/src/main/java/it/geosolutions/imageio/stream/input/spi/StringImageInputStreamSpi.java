 ...
 * to a {@link String}.
 *
 * <p>I basically rely on the existence of something to read from a {@link File} in case this {@link URL} points to a
 * {@link File}, otherwise I try to open up an {@link InputStream} and I ask the
 * {@link ImageIO#createImageInputStream(Object)} to create an {@link ImageInputStream} for it.
 *
...
public class StringImageInputStreamSpi extends ImageInputStreamSpi {
    ...
    /** @see javax.imageio.spi.ImageInputStreamSpi#createInputStreamInstance(java.lang.Object, boolean, java.io.File) */
    public ImageInputStream createInputStreamInstance(Object input, boolean useCache, File cacheDir)
            throws IOException {
        // is it a String?
        if (!(input instanceof String)) {
            if (LOGGER.isLoggable(Level.FINE)) LOGGER.fine("The provided input is not a valid String.");
            return null;
        }
        ...
        try {
            ...
            return new URLImageInputStreamSpi()
                    .createInputStreamInstance(tempURL, ImageIO.getUseCache(), ImageIO.getCacheDirectory());
        } catch (Throwable e) {
