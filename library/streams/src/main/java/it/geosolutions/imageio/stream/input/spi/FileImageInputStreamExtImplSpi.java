 ...
 * to a {@link File} by means of an {@link EnhancedRandomAccessFile} which gives buffering capabilities.
 *
 * <p>It is worth to point out that {@link ImageIO} already provide the {@link FileChannelImageInputStream} in order to
 * efficiently access images with buffering. despite to this I have ran into many problems with {@link FileChannel}s
 * especially on Windows machines, hence I came up with this {@link ImageInputStream} subclass and this
 * {@link ImageInputStreamSpi} which gives similar performances for most uses but far less problems.
 *
...
public class FileImageInputStreamExtImplSpi extends ImageInputStreamSpi {
    ...
    /**
     * Upon registration, this method ensures that this SPI is listed at the top of the ImageInputStreamSpi items, so
     * that it will be invoked before the default FileImageInputStreamSpi
     *
    ...
    public ImageInputStream createInputStreamInstance(Object input, boolean useCache, File cacheDir) {
        if (!(input instanceof File)) {
            if (LOGGER.isLoggable(Level.FINE)) LOGGER.fine("The provided input is not a eraf.");
            return null;
        }
