...
public class NetCDFUtilities {
    ...
    /**
     * Utility method to retrieve the t-index of a Variable descriptor stored on {@link NetCDFImageReader} NetCDF Flat
     * Reader {@link HashMap} indexMap.
     *
    ...
    public static int getTIndex(Variable var, Range range, int imageIndex) {
        ...
        if (rank > 2) {
            if (rank == 3) {
                ...
                return (int) Math.ceil((imageIndex - range.first()) / getZDimensionLength(var));
            }
        }
    ...
    /**
     * Utility method to retrieve the z-index of a Variable descriptor stored on {@link NetCDFImageReader} NetCDF Flat
     * Reader {@link HashMap} indexMap.
     *
     * @param var {@link Variable}
     * @param range {@link Range}
    ...
    public static String getAttributesAsString(final Variable var, final String attributeName) {
        ...
        if (var != null) {
            ...
            if (attribute != null) value = getAttributesAsString(attribute, false);
        }
        return value;
    }
    ...
    public static Number getAttributesAsNumber(final Variable var, final String attributeName) {
        ...
        if (var != null) {
            Attribute attribute = var.findAttribute(attributeName);
            if (attribute != null) value = attribute.getNumericValue();
        }
        return value;
    }

    /**
     * Return the value of a NetCDF {@code Attribute} instance as a {@code String}. The {@code isUnsigned} parameter
     * allow to handle byte attributes as unsigned, in order to represent values in the range [0,255].
     */
    ...
    /**
     * NetCDF files may contains a wide set of variables. Some of them are unuseful for our purposes. The method returns
     * {@code true} if the specified variable is accepted.
     */
    public static boolean isVariableAccepted(final Variable var, final CheckType checkType) {
        if (var instanceof CoordinateAxis1D) {
            ...
            return isVariableAccepted(var.getName(), CheckType.NONE);
        } else return isVariableAccepted(var.getName(), checkType);
    }
    ...
    public static boolean isVariableAccepted(final String name, final CheckType checkType) {
        if (checkType == CheckType.NONE) {
            if (name.equalsIgnoreCase(LATITUDE)
                    ...
                    || name.endsWith(BNDS)) return false;
            else return true;
        }
     ...
     * </ul>
     *
     * <p>The {@linkplain Locale#CANADA Canada locale} is used by default for most formats because it is relatively
    ...
    /**
     * Depending on the type of model/netcdf file, we will check for the presence of some variables rather than some
     * others. The method returns the type of check on which we need to leverage to restrict the set of interesting
     * variables. The method will check for some KEY/FLAGS/ATTRIBUTES within the input dataset in order to define the
    ...
    /**
     * Return a global attribute as a {@code String}. The required global attribute is specified by name
     *
     * @param attributeName the name of the required attribute.
     * @return the value of the required attribute. Returns an empty String in case the required attribute is not found.
     */
    ...
    public static KeyValuePair getGlobalAttribute(final NetcdfDataset dataset, final int attributeIndex)
            throws IOException {
        ...
        if (dataset != null) {
            ...
            if (globalAttributes != null && !globalAttributes.isEmpty()) {
                ...
                if (attribute != null) {
                    attributePair =
                            new KeyValuePair(attribute.getName(), NetCDFUtilities.getAttributesAsString(attribute));
                }
            }
        }
