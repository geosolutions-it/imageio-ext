...
/**
 * Class extending {@link GDALCommonIIOImageMetadata} in order to provide write capabilities to the metadata instance.
 * It is worth to point out that this class doesn't work on an underlying dataset. It simply allows to define a
...
public class GDALWritableCommonIIOImageMetadata extends GDALCommonIIOImageMetadata {
    ...
    public static final String nativeMetadataFormatName =
            "it.geosolutions.imageio.gdalframework.writablecommonImageMetadata_1.0";
     ...
     *
     * @param metadataNameValuePairs a <code>Map</code> containing name-value pairs where each pair represents a
     *     metadata element.
     * @param domain the domain where the metadata need to be stored.
     * @see GDALUtilities.GDALMetadataDomain <br>
     *     TODO: future version could check for already existent key or provide a step-to-step single metadata item
     *     setting
     * @throws IllegalArgumentException in case the specified domain is unsupported.
     */
    public synchronized void setGdalMetadataDomain(Map metadataNameValuePairs, String domain) {
        if (domain == null
                || domain.length() > 0
                        && (!domain.equals(GDALUtilities.GDALMetadataDomain.DEFAULT)
                                || !domain.equals(GDALUtilities.GDALMetadataDomain.IMAGESTRUCTURE)
                                || !domain.startsWith(GDALUtilities.GDALMetadataDomain.XML_PREFIX)))
            throw new IllegalArgumentException("Unsupported domain");
        if (domain.equals(GDALUtilities.GDALMetadataDomain.DEFAULT) || domain.length() == 0)
