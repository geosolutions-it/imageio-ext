...
public abstract class GDALImageReaderSpi extends ImageReaderSpi {
    ...
    private static final StringImageInputStreamSpi stringSPI = new StringImageInputStreamSpi();

    /** Cached instance of {@link URLImageInputStreamSpi} for creating streams out of {@link URL}s. */
    private static final URLImageInputStreamSpi URLSPI = new URLImageInputStreamSpi();
    ...
    public boolean canDecodeInput(Object input) throws IOException {
        ...
        if (input instanceof File) sourceFile = (File) input;

        // at this point it must be an instance of FileImageInputStreamExt to be able to proceed
        if (input instanceof FileImageInputStreamExt) sourceFile = ((FileImageInputStreamExt) input).getFile();
    ...
    /**
     * Checks if the provided Dataset was opened by a Driver supporting the same formats which are supported by the
     * specific ImageReaderSpi.
     *
     * <p>There is a trivial example: Suppose we are implementing a plugin for HDF4 format and suppose we are testing
     * the <code>canDecodeInput</code> with a NITF file as input. GDAL will successfully open the NITF file. However, it
     * will use the NITF driver instead of the HDF4 driver. Since NITF is not supported by the HDF4ImageReaderSpi, this
     * method will return return <code>false</code>.
     *
    ...
    public boolean isAvailable() {
        ...
        if (!GDALUtilities.isGDALAvailable()) return false;
        ...
        while (it.hasNext()) {
            ...
            if (!GDALUtilities.isDriverAvailable(formatName)) return false;
        }
        return true;
    }
