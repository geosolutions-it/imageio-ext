...
public class GDALCommonIIOImageMetadata extends IIOMetadata {
	...
	public static final String nativeMetadataFormatName = "org_gdal_imageio_common_image_metadata";
	...
	private Node getCommonNativeTree() {
		...
		if (dsWrapper.getColorModel() instanceof IndexColorModel) {
			...
			for (int i = 0; i < mapSize; i++) {
				node1 = new IIOMetadataNode("ColorTableEntry");
				node1.setAttribute("index", Integer.toString(i));
				node1.setAttribute("red", Byte.toString(rgb[0][i]));
				node1.setAttribute("green", Byte.toString(rgb[1][i]));
				node1.setAttribute("blue", Byte.toString(rgb[2][i]));
				node1.setAttribute("alpha", Byte.toString(rgb[3][i]));
				node.appendChild(node1);
			}
		}
		return root;
