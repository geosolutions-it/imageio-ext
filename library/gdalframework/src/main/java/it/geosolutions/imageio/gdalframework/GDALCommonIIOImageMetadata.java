...
/**
 * Class needed to store all available information of a GDAL Dataset with the add of additional information. For
 * convenience and future re-use this class also represents an {@link IIOMetadata}. A wide set of getters method allow
 * to retrieve several information directly from the metadata instance, without need of getting the XML DOM nodes tree.
 *
...
public class GDALCommonIIOImageMetadata extends CoreCommonImageMetadata {
    ...
    /**
     * A map containing an HashMap for each domain if available (the Default domain, the ImageStructure domain, as well
     * as any xml prefixed domain)
     */
    ...
    /**
     * <code>GDALCommonIIOImageMetadata</code> constructor. Firstly, it provides to open a dataset from the specified
     * input dataset name. Then, it call the constructor which initializes all fields with dataset properties, such as
     * raster size, raster tiling properties, projection, and more.
     *
     * @param sDatasetName The name (usually a File path or a subdataset name when the format supports subdatasets) of
     *     the dataset we want to open.
     */
     ...
     * @param sDatasetName The name (usually a File path or a subdataset name when the format supports subdatasets) of
     *     the dataset we want to open.
     * @param formatName the name of the native metadata format
     * @param formatClassName the name of the class of the native metadata format
     */
    public GDALCommonIIOImageMetadata(String sDatasetName, String formatName, String formatClassName) {
        this(
                GDALUtilities.acquireDataSet(sDatasetName, gdalconst.GA_ReadOnly),
                sDatasetName,
                true,
                formatName,
                formatClassName,
     ...
     *
     * @param dataset the input <code>Dataset</code> on which build the common metadata object.
     * @param name the name to be set for the dataset represented by this common metadata object.
     * @param initializationRequired specify if initializing fields is required or not.
     * @param formatName the name of the native metadata format
     * @param formatClassName the name of the class of the native metadata format
     */
    public GDALCommonIIOImageMetadata(
            Dataset dataset,
            String name,
    ...
    private GDALCommonIIOImageMetadata(
        ...
        try {
            ...
            //
            // //
            if (initializationRequired) setMembers(dataset);
    ...
    /**
     * Constructor which initializes fields by retrieving properties such as raster size, raster tiling properties,
     * projection, and more from a given input <code>Dataset</code>.
     *
     * @param dataset the <code>Dataset</code> used to initialize all the common metadata fields.
     * @param name the dataset name
     * @param formatName the name of the native metadata format
     * @param formatClassName the name of the class of the native metadata format
     */
    public GDALCommonIIOImageMetadata(
            final Dataset dataset, final String name, final String formatName, final String formatClassName) {
        this(dataset, name, true, formatName, formatClassName);
    }

    /**
     * Constructor which initializes fields by retrieving properties such as raster size, raster tiling properties,
     * projection, and more from a given input <code>Dataset</code> if not null.
     *
     * @param dataset the <code>Dataset</code> used to initialize all the common metadata fields.
     * @param name the dataset name
     */
     ...
     *
     * @param dataset a <code>Dataset</code> from where to retrieve all georeferencing information available
     */
     ...
     *
     * @param dataset the <code>Dataset</code> which will be used for the initialization
     * @return <code>true</code> if the initialization was successfully completed. <code>false</code> if some field
    ...
    private boolean setMembers(Dataset dataset) {
        ...
        try {
            ...
            if (((long) tileHeight) * ((long) tileWidth) > Integer.MAX_VALUE) performTileSizeTuning(dataset);
            ...
            // /////////////////////////////////////////////////////////////////
            int buffer_type = 0;
            if (buf_type == gdalconstConstants.GDT_Byte) buffer_type = DataBuffer.TYPE_BYTE;
            else if (buf_type == gdalconstConstants.GDT_UInt16) buffer_type = DataBuffer.TYPE_USHORT;
            else if (buf_type == gdalconstConstants.GDT_Int16) buffer_type = DataBuffer.TYPE_SHORT;
            else if ((buf_type == gdalconstConstants.GDT_Int32) || (buf_type == gdalconstConstants.GDT_UInt32))
                buffer_type = DataBuffer.TYPE_INT;
            else if (buf_type == gdalconstConstants.GDT_Float32) buffer_type = DataBuffer.TYPE_FLOAT;
            else if (buf_type == gdalconstConstants.GDT_Float64) buffer_type = DataBuffer.TYPE_DOUBLE;
            else return false;
            ...
            if (tileSize < 0)
                ...
                setSampleModel(new PixelInterleavedSampleModel(
                        buffer_type, tileWidth, tileHeight, numBands, tileWidth * numBands, bandsOffset));
            ...
            } else setColorModel(GDALUtilities.buildColorModel(getSampleModel()));

            if (getColorModel() == null || getSampleModel() == null) return false;
        } finally {
            if (rband != null) {
                try {
                    // Closing the band
    ...
    /**
     * Provides to increase data access performances. In many cases, the block size for a raster band is a single line
     * of N pixels, where N is the width of the raster.
     *
     * <p>The Java Advanced Imaging allows to load and manipulate data only when they are needed (The Deferred Execution
     * Model). This is done by working only on tiles containing the required data.
     *
     * <p>However, reading a big image composed of tiles having a size of Nx1 (The most commonly used block size) would
     * be not optimized because for each tile, a read operation is computed (very small tiles -> very high read
     * operations number)
     *
     * <p>In order to optimize data access operations, it would be better to make tiles a little bit greater than just a
     ...
     *
     * @param metadataDomain the requested GDAL metadata domain.
     * @see GDALUtilities.GDALMetadataDomain
     * @return the metadata mapping for the specified domain or <code>null</code> in case no metadata is available for
     *     the domain or the specified domain is unsupported.
     */
    protected Map getGdalMetadataDomain(final String metadataDomain) {
        if (metadataDomain.equalsIgnoreCase(GDALUtilities.GDALMetadataDomain.DEFAULT)) {
            ...
            if (gdalDomainMetadataMap.containsKey(metadataDomain)) return gdalDomainMetadataMap.get(metadataDomain);
        }
        return null;
    }
     ...
     *
     * @return a list of <code>String</code>s representing metadata domains defined for the dataset on which this
     *     instance is based.
     */
    ...
    /**
     * Returns a copy of this <code>GDALCommonIIOImageMetadata</code> as a <code>GDALWritableCommonIIOImageMetadata
     * </code> instance, with setting capabilities
     */
    /**
     * Returns a copy of this <code>GDALCommonIIOImageMetadata</code> as a <code>GDALWritableCommonIIOImageMetadata
     * </code> instance, with setting capabilities
    ...
    public GDALWritableCommonIIOImageMetadata asWritable() {
        ...
        if (cm != null) {
            if (cm instanceof IndexColorModel) {
                ...
                } else metadata.setColorModel(new IndexColorModel(icm.getPixelSize(), mapSize, r, g, b));
            } else metadata.setColorModel(GDALUtilities.buildColorModel(metadata.getSampleModel()));
        }
