 ...
 * href="http://www.gdal.org/">GDAL (Geospatial Data Abstraction Layer)</a> by means of SWIG (Simplified Wrapper and
 * Interface Generator) bindings in order to perform read operations.
 *
...
public abstract class GDALImageReader extends ImageReader {
    ...
    /** The optional URI referring the input source. As an instance, an ECWP link */
    private URI uriSource = null;

    /** {@link HashMap} containing couples (datasetName, {@link GDALCommonIIOImageMetadata}). */
    private ConcurrentHashMap<String, GDALCommonIIOImageMetadata> datasetMetadataMap =
            new ConcurrentHashMap<String, GDALCommonIIOImageMetadata>();
     ...
     *
     * @param imageIndex is the index of the required {@link GDALCommonIIOImageMetadata}.
    ...
    /**
     * Constructs a <code>GDALImageReader</code> using a {@link GDALImageReaderSpi}.
     *
     * @param originatingProvider The {@link GDALImageReaderSpi} to use for building this <code>GDALImageReader</code>.
     */
    ...
    public GDALImageReader(final GDALImageReaderSpi originatingProvider, final int numSubdatasets) {
        super(originatingProvider);
        if (numSubdatasets < 0) throw new IllegalArgumentException("The provided number of sub datasets is invalid");
    ...
    /**
     * Build a proper {@link GDALCommonIIOImageMetadata} given the name of a dataset. The default implementation return
     * a {@link GDALCommonIIOImageMetadata} instance.This method should be overridden by the specialized
     * {@link GDALImageReader} in case you need to obtain a specific {@link GDALCommonIIOImageMetadata}'s subclass
     *
     * @param datasetName the name of the dataset
     */
    protected GDALCommonIIOImageMetadata createDatasetMetadata(
            final Dataset ds, final String datasetName, boolean initializationRequired) {
     ...
     *
     * @param itemMetadata a <code>GDALCommonIIOImageMetadata</code> related to the actual dataset
    ...
    private Raster readDatasetRaster(
        ...
        try {
            ...
            if (LOGGER.isLoggable(Level.FINE)) LOGGER.fine("SourceRegion = " + srcRegion.toString());
            ...
            for (int k = 0; k < nBands; k++) {
                ...
                if (returnVal == gdalconstConstants.CE_None) {
                    ...
                    LOGGER.info(new StringBuilder("Last error: ")
                            .append(gdal.GetLastErrorMsg())
                            .toString());
                    LOGGER.info(new StringBuilder("Last error number: ")
                            .append(gdal.GetLastErrorNo())
                            .toString());
                    LOGGER.info(new StringBuilder("Last error type: ")
                            .append(gdal.GetLastErrorType())
                            .toString());
            ...
            if (splitBands)
                ...
                sampleModel = new PixelInterleavedSampleModel(
                        dataBufferType, dstWidth, dstHeight, nBands, dstWidth * nBands, offsets);
        } finally {
            if (pBand != null) {
                try {
                    // Closing the band
    ...
    protected File getDatasetSource(Object myInput) {
        if (datasetSource == null) {
            if (myInput instanceof File) datasetSource = (File) myInput;
                ...
                } else throw new IllegalArgumentException("Not a supported Input");
    ...
    public void setInput(Object input, boolean seekForwardOnly, boolean ignoreMetadata) {
        ...
        // check input
        if (input == null) throw new IllegalArgumentException("The provided input is null!");
        ...
        if (imageInputStream != null) {
            if (datasetSource != null) {
                ...
                mainDataSet =
                        GDALUtilities.acquireDataSet(datasetSource.getAbsolutePath(), gdalconstConstants.GA_ReadOnly);
        ...
        if (isInputDecodable) {
            ...
            } else sb.append("The Provided input is not supported by this reader");
     ...
     *
     * @param imageIndex the index of the image to be retrieved.
     * @param param an <code>ImageReadParam</code> used to control the reading process, or <code>null</code>. Actually,
     *     setting a destinationType allows to specify the number of bands in the destination image.
     * @return the desired portion of the image as a <code>BufferedImage</code>
     * @throws IllegalArgumentException if <code>param</code> contains an invalid specification of a source and/or
     *     destination band subset or of a destination image.
     * @throws IOException if an error occurs when acquiring access to the underlying datasource
     */
    public BufferedImage read(final int imageIndex, final ImageReadParam param) throws IOException {
        ...
        if (param == null) imageReadParam = getDefaultReadParam();
        else imageReadParam = param;
        ...
        if (imageType != null) {
            ...
            if (bi != null) nDestBands = bi.getSampleModel().getNumBands();
            else if (destBands != null) nDestBands = destBands.length;
            else if (srcBands != null) nDestBands = srcBands.length;
            else nDestBands = itemNBands;
        }
        ...
        if (bi == null) {
            ...
            if (imageType == null) {
                ...
                bi = new BufferedImage(
                        cm,
                        (WritableRaster) readDatasetRaster(item, srcRegion, destRegion, srcBands, null),
                        false,
                        null);
            } else {
                ...
                bi = new BufferedImage(
                        cm,
                        (WritableRaster) readDatasetRaster(item, srcRegion, destRegion, srcBands, destSampleModel),
                        false,
                        null);
            }
            ...
            Raster readRaster = readDatasetRaster(item, srcRegion, destRegion, srcBands, null);
    ...
    /**
     * Implements the <code>ImageRead.readRaster</code> method which returns a new <code>Raster</code> object containing
     * the raw pixel data from the image stream, without any color conversion applied.
     *
     * @param imageIndex the index of the image to be retrieved.
    ...
    public BufferedImage read(int imageIndex) throws IOException {
        if (LOGGER.isLoggable(Level.FINE)) LOGGER.fine("read(imageIndex)");
        return read(imageIndex, null);
    }

    /**
     * Returns the number of images (subdatasets) contained within the data source. If there are no subdatasets, it
     * returns 1.
     */
    public int getNumImages(boolean allowSearch) throws IOException {
        if (LOGGER.isLoggable(Level.FINE)) LOGGER.fine("getting NumImages");
        return nSubdatasets;
    }

    /**
     * Returns the width of the raster of the <code>Dataset</code> at index <code>imageIndex</code>.
     *
     * @param imageIndex the index of the specified raster
     * @return raster width
     */
    ...
    /**
     * Returns the height of the raster of the <code>Dataset</code> at index <code>imageIndex</code>.
     *
     * @param imageIndex the index of the specified raster
    ...
    /**
     * Returns the tile height of the raster of the <code>Dataset</code> at index <code>imageIndex</code>.
     *
     * @param imageIndex the index of the specified raster
    ...
    /**
     * Returns the tile width of the raster of the <code>Dataset</code> at index <code>imageIndex</code>.
     *
     * @param imageIndex the index of the specified raster
    ...
    /**
     * Retrieves the WKT projection <code>String</code> for the <code>Dataset</code> at index <code>imageIndex</code>.
     *
     * @param imageIndex the index of the dataset we want to get the projections for.
     * @return the WKT projection <code>String</code> for the <code>Dataset</code> at index <code>imageIndex</code>.
     */
    ...
    /**
     * Retrieves the GeoTransformation coefficients for the <code>Dataset</code> at index <code>imageIndex</code>.
     *
     * @param imageIndex the index of the dataset we want to get the coefficients for.
    ...
    /**
     * Returns Ground Control Points of the <code>Dataset</code> at index <code>imageIndex</code>.
     *
     * @param imageIndex the index of the specified <code>Dataset</code>
    ...
    /**
     * Returns the Ground Control Points projection definition string of the <code>Dataset</code> at index <code>
     * imageIndex</code>.
     *
     * @param imageIndex the index of the specified <code>Dataset</code>
    ...
    /**
     * Returns the number of Ground Control Points of the <code>Dataset</code> at index imageIndex.
     *
     * @param imageIndex the index of the specified <code>Dataset</code>
     ...
     *
     * @param imageIndex the specified image
     * @param band the specified band
     * @return the Band NoDataValue if available
     * @throws IllegalArgumentException in case the specified band number is out of range or noData value has not been
     *     found
     */
    ...
    /**
     * Returns the optional Offset Value of the specified band of the <code>Dataset</code> at index <code>imageIndex
     * </code>.
     *
     * @param imageIndex the index of the specified <code>Dataset</code>
     * @param band the specified band
     * @return the Band Offset Value if available
     * @throws IllegalArgumentException in case the specified band number is out of range or Offset value has not been
     *     found
     */
    ...
    /**
     * Returns the optional Scale Value of the specified band of the <code>Dataset</code> at index <code>imageIndex
     * </code>.
     *
     * @param imageIndex the index of the specified <code>Dataset</code>
     * @param band the specified band
     * @return the Band Scale Value if available
     * @throws IllegalArgumentException in case the specified band number is out of range or scale value has not been
     *     found
     */
    ...
    /**
     * Returns the optional Minimum Value of the specified band of the <code>Dataset</code> at index <code>imageIndex
     * </code>.
     *
     * @param imageIndex the index of the specified <code>Dataset</code>
     * @param band the specified band
     * @return the Band Minimum Value if available
     * @throws IllegalArgumentException in case the specified band number is out of range or minimum value has not been
     *     found
     */
    ...
    /**
     * Returns the optional Maximum Value of the specified band of the <code>Dataset</code> at index <code>imageIndex
     * </code>.
     *
     * @param imageIndex the index of the specified <code>Dataset</code>
     * @param band the specified band
     * @return the Band Maximum Value if available
     * @throws IllegalArgumentException in case the specified band number is out of range or maximum value has not been
     *     found
     */
    ...
    /**
     * Returns an <code>IIOMetadata</code> object containing metadata associated with the given image, specified by the
     * <code>imageIndex</code> parameter
     *
