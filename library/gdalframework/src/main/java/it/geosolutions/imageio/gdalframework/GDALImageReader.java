	...
	public GDALDatasetWrapper getDataSetWrapper(int imageIndex) {
		...
		synchronized (datasetMap) {
			...
			if (datasetMap.containsKey(datasetName))
				...
				// Should never happen since initialization has already been
				// computed.
				throw new IllegalArgumentException(
						"No Dataset found for the specified imageIndex "
								+ imageIndex);
			// Add a new GDALDatasetWrapper to the HashMap
			// return (GDALDatasetWrapper) datasetMap.put(datasetName,
			// new GDALDatasetWrapper(datasetName));
		}
	}
