...
public class ImageIOUtilities {
     ...
     * </code>. Data with 5 or more bands will have a <code>BogusColorSpace</code>.
     *
     ...
     * included in the computation of the future JAI extrema operation used before the JAI rescale operation.
     *
                ...
                if (formats[i].equalsIgnoreCase(format)) {
                    ...
                    if (standard != null && custom != null) {
                        if (ImageReaderSpi.class.isAssignableFrom(standard.getClass()))
                            return registry.setOrdering(
                                    ImageReaderSpi.class, (ImageReaderSpi) custom, (ImageReaderSpi) standard);
                        else
                            return registry.setOrdering(
                                    ImageWriterSpi.class, (ImageWriterSpi) custom, (ImageWriterSpi) standard);
                    }
                }
            }
        }
     ...
     * JAI 1.1.2, the {@code "Affine"} operation on an image with float data type, bilinear interpolation and an
    ...
    public static ImageTypeSpecifier getBandSelectedType(int numBands, SampleModel sm) {
        ColorModel cmDestination = new ComponentColorModel(
                new BogusColorSpace(numBands), false, false, Transparency.OPAQUE, sm.getDataType());
