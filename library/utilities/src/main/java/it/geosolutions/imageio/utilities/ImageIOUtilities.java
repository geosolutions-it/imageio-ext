...
public class ImageIOUtilities {
    ...
    private static boolean SKIP_EXTERNAL_FILES_LOOKUP =
            Boolean.getBoolean("it.geosolutions.skip.external.files.lookup");

    static final int MAX_SUBSAMPLING_FACTOR = Integer.MAX_VALUE;
    static final int MAX_LEVELS = 31;
    /**
     * An array of strings containing only white spaces. Strings' lengths are equal to their index + 1 in the
     * {@code spacesFactory} array. For example, {@code spacesFactory[4]} contains a string of length 5. Strings are
     ...
     * </code>. For 1- and 3- banded <code>SampleModel</code>s, the returned <code>ColorModel</code> will be opaque. For
     * 2- and 4-banded <code>SampleModel</code>s, the output will use alpha transparency which is not premultiplied. 1-
     * and 2-banded data will use a grayscale <code>ColorSpace</code>, and 3- and 4-banded data a sRGB <code>ColorSpace
     * </code>. Data with 5 or more bands will have a <code>NotAColorSpace</code>.
     *
     * <p>An instance of <code>DirectColorModel</code> will be created for instances of <code>
     * SinglePixelPackedSampleModel</code> with no more than 4 bands.
     *
     * <p>An instance of <code>IndexColorModel</code> will be created for instances of <code>MultiPixelPackedSampleModel
     * </code>. The colormap will be a grayscale ramp with <code>1&nbsp;<<&nbsp;numberOfBits</code> entries ranging from
     * zero to at most 255.
     *
        ...
        if (sampleModel instanceof ComponentSampleModel) {
            ...
            if (dataType == DataBuffer.TYPE_SHORT)
                colorModel = new ComponentColorModel(
                        colorSpace, sampleSize, hasAlpha, isAlphaPremultiplied, transparency, dataType);
            else
                colorModel = RasterFactory.createComponentColorModel(
                        dataType, colorSpace, hasAlpha, isAlphaPremultiplied, transparency);
    ...
    /**
     * Given a root node, print the values/attributes tree using the System Out @TODO change it using Logger
     *
    ...
    /**
     * Visualize the image, after rescaling its values, given a threshold for the ROI. This is useful to rescale an
     * image coming from a source which may contain noDataValues. The ROI threshold allows to set a minimal value to be
     * included in the computation of the future ImageN extrema operation used before the ImageN rescale operation.
     *
     * @param image RenderedImage to visualize
     * @param title title for the frame
     * @param roiThreshold the threshold for the inner ROI
     */
     ...
     *
     * @param ri RenderedImage to visualize
     */
     ...
     *
     * @param ri RenderedImage to visualize
     * @param title title for the frame (usually the image filename)
     */
     ...
     * @param ri RenderedImage to visualize
     * @param title title for the frame (usually the image filename)
     * @param rescale if <code>true</code> the RenderedImage will be rescaled using a default value for the ROI.
     */
    public static void visualize(final RenderedImage ri, String title, final boolean rescale) {
     ...
     * @param title title for the frame (usually the image filename)
     * @param rescale if <code>true</code> the RenderedImage will be rescaled using a default value for the ROI.
     * @param roiThreshold the threshold for the inner ROI. It represent the minimum value of the pixels representing
     *     the region of interest.
     */
    public static void visualize(RenderedImage ri, String title, boolean rescale, int roiThreshold) {
        if (rescale == true) visualizeRescaled(ri, title, roiThreshold);
        else visualize(ri, title);
    }
    ...
    public static List<ImageReaderWriterSpi> getJDKImageReaderWriterSPI(
        ...
        final Iterator<? extends ImageReaderWriterSpi> iter =
                iioRegistry.getServiceProviders(spiClass, true); // useOrdering
    ...
    public static List<ImageReaderWriterSpi> getImageReaderWriterSPI(
            ServiceRegistry registry, ServiceRegistry.Filter filter, String formatName, boolean isReader) {
        ...
        final Iterator<? extends ImageReaderWriterSpi> iter =
                iioRegistry.getServiceProviders(spiClass, filter, true); // useOrdering
    ...
    /**
     * Replace the original provider with name originalProviderName with the provider with name customProviderName for
     * the class providerClass and for the provided format .
     *
     ...
     * @return <code>true</code> if we find both of the providers and the replacement succeed, <code>false</code>
     *     otherwise.
     */
    public static boolean replaceProvider(
        ...
        for (final Iterator<? extends ImageReaderWriterSpi> it = registry.getServiceProviders(providerClass, false);
                it.hasNext(); ) {
        ...
        if (standard != null && custom != null) {
            ...
            else return registry.setOrdering(ImageWriterSpi.class, (ImageWriterSpi) custom, (ImageWriterSpi) standard);
        }
     ...
     * interfaces exist for different coordinate system geometries ({@code CartesianCS}, {@code PolarCS}, etc.).
     */
    public static boolean sameInterfaces(final Class<?> object1, final Class<?> object2, final Class<?> base) {
        if (object1 == object2) {
            return true;
        }
     ...
     *
     * <p>Spits an {@link IllegalArgumentException} in case the data type is unknown.
     *
     * @param dataType {@link DataBuffer} type.
     * @return the class of the native type for the specified {@link DataBuffer} type or an
    ...
    /**
     * Given a pair of xSubsamplingFactor (xSSF) and ySubsamplingFactor (ySFF), look for a subsampling factor (SSF) in
     * case xSSF != ySSF or they are not powers of 2. In case xSSF == ySSF == 2^N, the method return 0 (No optimal
     * subsampling factor found).
     *
    ...
    public static int getSubSamplingFactor2(final int xSubsamplingFactor, final int ySubsamplingFactor) {
        ...
        if (newSubSamplingFactor > MAX_SUBSAMPLING_FACTOR) newSubSamplingFactor = MAX_SUBSAMPLING_FACTOR;
    ...
    static int findOptimalSubSampling(final int newSubSamplingFactor) {
        ...
        for (int level = 0; level < MAX_LEVELS; level++) {
            ...
            if (optimalSubSamplingFactor < newSubSamplingFactor) optimalSubSamplingFactor = 1 << level;
    ...
    /**
     * Allows or disallow native acceleration for the specified operation on the given ImageN instance. By default,
     * ImageN uses hardware accelerated methods when available. For example, it make use of MMX instructions on Intel
     * processors. Unluckily, some native method crash the Java Virtual Machine under some circumstances. For example on
     * ImageN, the {@code "Affine"} operation on an image with float data type, bilinear interpolation and an
     ...
     *     {@linkplain JAI#getDefaultInstance default ImageN instance}.
     ...
     * default ImageN instance}. This method is a shortcut for <code>
    ...
    public static final void checkNotNull(final Object checkMe, final String message) {
        if (checkMe == null) {
     ...
     *
     * @param rOp the image to be disposed.
     */
    ...
    /**
     * Returns a {@link ImageTypeSpecifier} usable for the given number of bands, and compatible wtih a base image type
     * (will reuse the data type and sample model width/height)
     */
    public static ImageTypeSpecifier getBandSelectedType(int numBands, SampleModel sm) {
        ColorModel cmDestination = new ComponentColorModel(
                new NotAColorSpace(numBands), false, false, Transparency.OPAQUE, sm.getDataType());
        SampleModel smDestination =
                RasterFactory.createComponentSampleModel(sm, sm.getDataType(), sm.getWidth(), sm.getHeight(), numBands);
        return new ImageTypeSpecifier(cmDestination, smDestination);
    }

    /**
     * Convert an object to a string. If the object is an array of primitive types (byte[], int[], short[]) it will
     * convert each element to a string and concatenate them with a space. If the object is null it will return an empty
     * string. For other object types it will call the toString() method.
     */
    ...
    /**
     * Checks if the provided image is contiguous in memory. This is true for binary images, or for images with a
     * ComponentSampleModel with pixelStride equal to numBands, bandOffsets equal to {0,1,2,...} and all bankIndices
     * equal to 0.
     */
    ...
    /**
     * Extracts packed binary data (1 bit per pixel) from a binary Raster into a byte array. The data is packed with 8
     * pixels per byte, with the most significant bit corresponding to the leftmost pixel.
     *
     * <p>If the Raster is not binary (1 bit per pixel, 1 band) an IllegalArgumentException is thrown.
     */
                        ...
                        for (int y = 0; y < rectHeight; ++y) {
                            ...
                            for (int xRemaining = rectWidth; xRemaining > 0; xRemaining -= 8) {
                                if (xRemaining > rightShift) {
                                    binaryDataArray[b++] =
                                            (byte) ((data[i++] & 255) << leftShift | (data[i] & 255) >>> rightShift);
                                } else {
    ...
    /**
     * Sets packed binary data (1 bit per pixel) into a binary WritableRaster from a byte array. The data is packed with
     * 8 pixels per byte, with the most significant bit corresponding to the leftmost pixel.
     *
     * <p>If the Raster is not binary (1 bit per pixel, 1 band) an IllegalArgumentException is thrown.
     */
                            ...
                            for (int xRemaining = rectWidth; xRemaining > 0; xRemaining -= 8) {
                                ...
                                if (xRemaining > leftShift8) {
                                    ...
                                    data[i] = (byte) (data[i] & (mask | remainMask)
                                            | (datum & 255) >>> rightShift & ~remainMask);
                                }
                            }
     ...
     *
     * <p>If the provided ImageReadParam contains a destination type, it is checked against the provided imageTypes
     * iterator. If it is compatible with one of the types in the iterator, it is returned, otherwise an IIOException is
     * thrown.
     *
     * <p>If the ImageReadParam does not contain a destination type, the first type from the imageTypes iterator is
     * returned.
     *
     * <p>If the imageTypes iterator is null or empty, an IllegalArgumentException is thrown.
     *
     * <p>
     *
     * @param param
     ...
     * @throws IIOException
     */
    public static final ImageTypeSpecifier getDestinationType(
            ImageReadParam param, Iterator<ImageTypeSpecifier> imageTypes) throws IIOException {
    ...
    /**
     * Checks if the provided ColorSpace is a non-standard ICC based color space. Non-standard means that it is not
     * sRGB, Gray, YCbCr or CMYK.
     *
     * <p>This method can be used to check if color conversion should be applied when converting an image to sRGB for
     * display purposes.
     */
    ...
    /**
     * Checks if the provided ImageWriter can encode images of the provided type. If the type is null, or if the
     * writer's originating provider is null, no exception is thrown.
     *
     * <p>If the writer cannot encode images of the provided type, an IIOException is thrown.
     */
    public static void canEncodeImage(ImageWriter writer, ImageTypeSpecifier type) throws IIOException {
        ...
        if (type != null && spi != null && !spi.canEncodeImage(type)) {
            throw new IIOException(String.format(
                    "Writer %s cannot encode images of type %s", spi.getDescription(Locale.getDefault()), type));
        }
    }
     ...
     *
     * <p>The size is computed as scanlineStride * height * elementSizeInBytes, plus any band or pixel offsets.
     *
     * <p>If the SampleModel is null, 0 is returned.
     */
     ...
     * Returns the size in bytes of a tile for the provided SampleModel.
     *
     * <p>The size is computed as width * height * numBands * elementSizeInBytes
     *
     * <p>If the SampleModel is null, 0 is returned.
     */
