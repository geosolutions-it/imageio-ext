...
/**
 * {@link JP2KBox} which is loaded lazily, which means only when the content is needed.
 *
...
class LazyJP2KBox extends DefaultMutableTreeNode implements JP2KBoxMetadata {
     ...
     * @param locator the JP2 locator referring this JP2K Box.
     */
    LazyJP2KBox(final String filename, final int type, final Jp2_locator locator) {
    ...
    public byte[] getContent() {
        ...
        // paranoid check
        if (BoxUtilities.SUPERBOX_NAMES.contains(BoxUtilities.getTypeString(type))) return null;
    ...
    static JP2KBox getAsOriginalBox(JP2KBox box) {
        ...
        if (box instanceof LazyJP2KBox) returnedBox = (JP2KBox) ((LazyJP2KBox) box).getOriginalBox();
        else if (box instanceof JP2KBox) returnedBox = (JP2KBox) box;
        else throw new IllegalArgumentException("Not a valid JP2Box");
        return returnedBox;
    }
