 ...
 *
 * <p>Note that for the moment we are doing basic parsing which means that we try to recognize as many boxes as possible
 * but we uses the {@link Jp2_family_src} object from kakadu. In the future we should allow this file walker to walk
...
class JP2KFileWalker {
    ...
    /** Tells me whether or not this {@link JP2KFileWalker} has been initialized or not. */
    private boolean initialized;

    /** Contains all the boxes this {@link JP2KFileWalker} has been able to recognize, parsed in a tree model. */
    ...
    private void init() {
        ...
        try {
            ...
            try {
                if (inputBox != null) inputBox.Close();
     ...
     *
     * @param inputBox the box to begin the parsing with.
     * @param parent the parent box to which attach this box as well as its brothers.
     * @param index the index at which this box should be attached in his father's children list.
     * @throws KduException in case any problem with the low level machinery happens.
     */
    private void parse(final Jp2_input_box inputBox, final JP2KBox parent, int index) throws KduException {
        ...
        try {
            ...
            // clean up
            try {
                if (childBox != null) childBox.Close();
    ...
    /**
     * Retrieves a cached {@link TreeModel} instance which represent the underlying jpeg2k file.
     *
     * @return a cached {@link TreeModel} instance which represent the underlying jpeg2k file.
     */
