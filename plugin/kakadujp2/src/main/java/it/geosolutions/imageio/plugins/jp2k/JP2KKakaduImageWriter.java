...
public class JP2KKakaduImageWriter extends ImageWriter {
    ...
    private static final short[] GEOJP2_UUID =
            new short[] {0xb1, 0x4b, 0xf8, 0xbd, 0x08, 0x3d, 0x4b, 0x43, 0xa5, 0xae, 0x8c, 0xd7, 0xd5, 0xa6, 0xce, 0x03
            };

    private static final int POWERS_2[] = new int[] {
    ...
    private static final int DEFAULT_MAX_BUFFER_SIZE = 32 * 1024 * 1024;

    /** The Default Temp Buffer Size */
    private static final int DEFAULT_TEMP_BUFFER_SIZE = 64 * 1024;

    /** Set at once to indicate whether to write the comment marker generated by rate allocation */
    private static final boolean ADD_COMMENT_MARKER;

    /** A temporary buffer size where to store data when directly writing on an output stream */
    ...
    private static final class MarkerSize {
        ...
        static final int TLM = 8; // Value FF55 + L_TLM bytes + I_TLM + L
    ...
    /**
     * In case the ratio between the stripe_height and the image height is greater than this value, set the
     * stripe_height to the image height in order to do a single push
     */
    ...
    public void write(IIOMetadata streamMetadata, IIOImage image, ImageWriteParam param) throws IOException {
        ...
        try {
            ...
            bytesOverHead += addMarkerBytes(
                    nComponents, destinationWidth, destinationHeight, tileW, tileH, orgGen_tlm, addCommentMarker);
            if (bytesOverHead >= imageSize) {
                bytesOverHead = 0;
            }

            final long qualityLayersSize = bitRates != null
                    ? imageSize
            ...
            throw (IOException)
                    new IOException("Error caused by a Kakadu exception during write operation").initCause(e);
        } finally {
            // //
            //
            ...
            if (!writeCodeStreamOnly && target != null) {
                ...
                if (familyTarget != null) {
                    try {
                        if (familyTarget.Exists()) familyTarget.Close();
                    } catch (Throwable e) {
                        // Does Nothing
                    }
                    try {
     ...
     * @throws KduException
     */
    private void pushStripes(
            final Kdu_stripe_compressor compressor,
            final RenderedImage inputRenderedImage,
            final Rectangle imageBounds,
            final Rectangle originalBounds,
            final boolean isDataSigned,
            final boolean resampleInputImage,
            final boolean writeCodeStreamOnly,
            ...
            final int[] stripeHeights,
            final int[] sampleGaps,
            final int[] rowGaps,
            final int[] sampleOffsets,
        ...
        if (bits <= 8) {
            ...
            if (!resampleInputImage) {
                ...
                while (goOn) {
                    ...
                    goOn = compressor.Push_stripe(
                            bufferValues, stripeHeights, sampleOffsets, sampleGaps, rowGaps, precisions, 0);
                    y += stripeHeights[0];
                    rasterData = null;
                }
            } else {
                ...
                while (goOn) {
                    if (destinationHeight - y < stripeHeights[0]) {
                        for (int i = 0; i < nComponents; i++) stripeHeights[i] = destinationHeight - y;
                        buffer = ByteBuffer.allocate(rowSize * stripeHeights[0]);
                    }
                    Rectangle rect = new Rectangle(
                            imageBounds.x, lastY, imageBounds.width, stripeHeights[0] * ySubsamplingFactor);
                    ...
                    goOn = compressor.Push_stripe(
                            data.array(), stripeHeights, sampleOffsets, sampleGaps, rowGaps, precisions, 0);
            ...
            for (int i = 0; i < isSigned.length; i++) isSigned[i] = isDataSigned;
            ...
            if (!resampleInputImage) {
                while (goOn) {
                    ...
                    Raster rasterData =
                            inputRenderedImage.getData(new Rectangle(0, y, destinationWidth, stripeHeights[0]));
                    rasterData.getDataElements(0, y, destinationWidth, stripeHeights[0], bufferValues);
                    goOn = compressor.Push_stripe(
                            bufferValues, stripeHeights, sampleOffsets, sampleGaps, rowGaps, precisions, isSigned, 0);
                    y += stripeHeights[0];
                    rasterData = null;
                }
            } else {
                ...
                while (goOn) {
                    ...
                    goOn = compressor.Push_stripe(
                            buffer.array(), stripeHeights, sampleOffsets, sampleGaps, rowGaps, precisions, isSigned, 0);
            ...
            while (goOn) {
                if (!resampleInputImage) {
                    ...
                    rasterData.getDataElements(0, y, destinationWidth, stripeHeights[0], bufferValues);
                    goOn = compressor.Push_stripe(
                            bufferValues, stripeHeights, sampleOffsets, sampleGaps, rowGaps, precisions);
                    y += stripeHeights[0];
                    rasterData = null;
                } else {
                    int lastY = imageBounds.y;
                    ...
                    while (goOn) {
                        ...
                        goOn = compressor.Push_stripe(
                                buffer.array(), stripeHeights, sampleOffsets, sampleGaps, rowGaps, precisions);
    ...
    private int addMarkerBytes(
        ...
        bytesOverhead += MarkerSize.SOC
                + siz_size
                + MarkerSize.COD
                + MarkerSize.QCD_ESTIMATE
                + (addComment ? MarkerSize.COM : MarkerSize.COM_KAKADUV)
                + MarkerSize.SOT
                + MarkerSize.SOD
                + MarkerSize.EOC;
        if (tileW != Integer.MIN_VALUE
                && tileH != Integer.MIN_VALUE
                && tileW != 0
                && tileH != 0
                && orgGen_tlm != JP2KKakaduImageWriteParam.UNSPECIFIED_ORG_GEN_TLM) {
     ...
     *
     * @param codeStream the output codestream
     * @param ycc the cycc parameter
     * @param cLevels the number of DWT decomposition levels
     * @param quality the quality factor (in the range 0.01 - 1. where 1 = LOSSLESS)
     * @param qualityLayers the number of quality layers
     * @param colorModel the input image's colorModel
     * @param writeCodeStreamOnly <code>true</code> if we need to write only codestream.
     * @param target the optional {@link Jp2_target} object in case we are writing only codestream.
     * @param orgGen_plt if {@code true}, request the insertion of packet length information in the header of
     *     tile-parts.
     * @param orgGen_tlm the Tile-part-lenght (TLM) marker marker segments in the main header.
     * @param orgT_parts parameter setting the division of each tile's packets into tile-parts
     * @param qGuard parameter setting the guard bits
     * @param cOrder the {@link ProgressionOrder} to be used.
     * @param cPrecincts the optional Precincts settings
     * @return {@code false} in case we aren't writing codestream only and a proper {@link Jp2_target} haven't been
     *     provided
     * @throws KduException
     */
    private boolean initializeCodestream(
            final Kdu_codestream codeStream,
            final boolean ycc,
            final int cLevels,
            final double quality,
            final int qualityLayers,
            final ColorModel colorModel,
            final boolean writeCodeStreamOnly,
            final int dataType,
            final Jp2_target target,
            final byte[] geoJp2,
            final boolean orgGen_plt,
            final int orgGen_tlm,
            final String orgT_parts,
            final int qGuard,
            final ProgressionOrder cOrder,
            final String cPrecincts,
     ...
     *
     * @param compressor the {@link Kdu_stripe_compressor} to be initialized.
     * @param codeStream the {@link Kdu_codestream}
     * @param quality the requested quality factor
     * @param qualityLayers the number of specified quality layers
     * @param qualityLayersSize the requested quality layers total size.
     * @param bitRates
     * @param compression
     * @param rowSize the size of a destination image row in pixels
     * @param destinationHeight the destination image height
     * @param destinationWidth the destination image width
     * @param nComponents the number of components
     * @param addCommentMarker
     * @param stripeHeights the stripeHeights array
     * @param sampleGaps the sampleGaps array
     * @param rowGaps the rowGaps array
     * @param sampleOffsets the sampleOffsets array
     * @param precisions the precisions array
     * @param numberOfBits the bit depth array
     * @throws KduException
     */
    private void initializeStripeCompressor(
            ...
            final double quality,
            final int qualityLayers,
            final long qualityLayersSize,
            final double[] bitRates,
            final Compression compression,
            final int rowSize,
            final int destinationHeight,
            final int destinationWidth,
        ...
        compressor.Start(
                ...
                nComponents,
                false);
        final boolean useRecommendations =
                compressor.Get_recommended_stripe_heights(minStripeHeight, maxStripeHeight, stripeHeights, null);
    ...
    /**
     * Given a requested number of qualityLayers, computes the cumulative quality layers sizes to be set as argument of
     * the stripe_compressor.
     *
     * @param qualityLayers the number of quality layers
     * @param referenceSize the total amount of bytes used by the quality layers.
     * @param bitRates
     * @return a <code>long</code> array containing the cumulative layers sizes.
     */
    private long[] computeQualityLayers(final int qualityLayers, final long referenceSize, final double[] bitRates) {
        long[] cumulativeQualityLayerSizes = new long[qualityLayers];
        if (bitRates != null) {
            // Use explicit bitRates provided.
            for (int i = 0; i < qualityLayers; i++) {
                cumulativeQualityLayerSizes[i] =
                        (long) Math.floor(bitRates[i] * KakaduUtilities.BIT_TO_BYTE_FACTOR * (double) referenceSize);
            }
     ...
     *
     * @param target the {@link Jp2_target} object.
     * @param sizParams the {@link Siz_params} containing information needed for initialization.
     * @param colorModel the color model of the image to be written.
     * @throws KduException
     */
    private void initializeHeader(final Jp2_target target, final Siz_params sizParams, final ColorModel colorModel)
            throws KduException {
    ...
    /**
     * Read a region of an input raster and put the data values in the provided Buffer.
     *
     * @param region the requested region
     * @param originalBounds the original Image Extent
     * @param lastX the last pixel to be analyzed along the image width.
     * @param lastY the last pixel to be analyzed along the image height.
     * @param xSubsamplingFactor the subsampling factor along the image width.
     * @param ySubsamplingFactor the subsampling factor along the image height.
     * @param rasterData the original raster data.
     * @param dataBuffer the buffer containing the data read.
     * @param nComponents the number of image components.
     */
    private void readSubSampled(
    ...
    /**
     * Initialize the {@link Siz_params} object with the basic set of provided information, such as image size, number
     * of components, bit depth, data sign and tiling.
     *
     * @param params the {@link Siz_params} to be set.
     * @param width the image width
     * @param height the image height
     * @param precision the number of bits of each component.
     * @param components the number of components
     * @param isSigned {@code true} if data is signed.
     * @param tileW the requested inner tiling width
     * @param tileH the requested inner tiling height
     * @throws KduException
     */
