 ...
 *     <p>Utility class providing methods to setup/parse EXIF, write it to stream, retrieve from stream.
 */
public class EXIFUtilities {
    ...
    /** Simple dummy class to wrap an EXIFMetadata instance as well as the length of the APP1 marker. */
    ...
    static final byte _0 = 0x00;

    static final byte FF = (byte) 0xFF;

    /** EXIF Marker identifier */
    static final byte[] EXIF_MARKER = new byte[] {'E', 'x', 'i', 'f', _0, _0};

    /** Offset to be appended before starting IFD1 content */
    static final byte[] NEXT_IFD = new byte[] {_0, _0, _0, _0};

    /** BIG Endian TIFF HEADER */
    static final byte[] TIFF_HEADER = new byte[] {'M', 'M', _0, 0x2A, _0, _0, _0, 8};

    static final int TIFF_HEADER_LENGTH = TIFF_HEADER.length;

    /**
     * UserComment ASCII character code prefix to be inserted before any UserComment String when writing it into the
     * EXIF marker
     */
    static final byte[] USER_COMMENT_ASCII_CHAR_CODE = new byte[] {0x41, 0x53, 0x43, 0x49, 0x49, _0, _0, _0};

    /** The APP1 Marker bytes, (APP1 is the one containing EXIF) */
    static final byte[] APP1_MARKER = new byte[] {FF, (byte) 0xE1};

    /**
     * The Data Quantization Table Marker. It is contained within a JPEG encoded data stream before the data image. EXIF
     * marker should be put before this marker
     */
    static final byte[] DQT_MARKER = new byte[] {FF, (byte) 0xDB};
    ...
    static final byte[] NULL_STRING = new byte[] {_0};

    /**
     * The fixed length of each IFD, made of Number (2 bytes) + Type (2 bytes) + Count (4 bytes) + Value/Offset (4
     * Bytes)
     */
    static final int IFD_LENGTH = 12;

    /**
     * This method will update the image referred by the specified inputStream, by replacing the underlying EXIF with
     * the one represented by the specified {@link EXIFMetadata} instance. Write the result to the specified
     * {@link OutputStream}.
     *
     * @param outputStream the stream where to write the result
     * @param inputStream the input stream referring to the original image to be copied back to the output
     * @param exif the {@link EXIFMetadata} instance containing updated exif to replace the one contained into the input
     *     image.
     * @param previousEXIFLength the length of the previous EXIF marker. It is needed in order to understand the portion
     *     of the input image to be copied back to the output
     * @throws IOException
     */
    ...
    /**
     * This method allows to parse the provided {@link EXIFMetadata} object and put it into the specified outputStream
    ...
    public static void insertEXIFintoStream(
            final OutputStream outputStream, final byte[] imageData, final int imageDataSize, final EXIFMetadata exif)
            throws IOException {
        ByteArrayOutputStream baos = null;
    ...
    /**
     * This method write the provided {@link EXIFMetadata} into the specified outputStream while copying back the JPEG
     * encoded image referred by the imageData argument.
     *
     * @param outputStream the stream where to write
     * @param imageData the bytes containing JPEG encoded image data
     * @param imageDataSize the number of bytes to be used from the data array
    ...
    /**
     * Write the specified {@link EXIFMetadata} object as well as the specified image data bytes to the specified
     * outputStream. It will take care of inserting the EXIF marker in the proper location within the outputStream when
     * writing image data bytes.
     *
     ...
     * @param outputStream an optional {@link ByteArrayOutputStream} where to write the exif marker. If null, a new
     *     {@link ByteArrayOutputStream} will be created and returned
    ...
    private static ByteArrayOutputStream initializeExifStream(
            final EXIFMetadata exif, final ByteArrayOutputStream outputStream) throws IOException {
        ...
        computeOffsetsAndSizes(
                exifTags,
                baseLineTagsOffsets[numBaselineTags - 1] + BYTES_FOR_TAGS_NUMBER,
                exifTagsOffsets,
                exifTagsContentSizes);
        ...
        int app1Lenght = APP1_MARKER.length
                ...
                + numBaselineTags * IFD_LENGTH
                + numExifTags * IFD_LENGTH // Bytes needed to represent all IFD
                + baselineContentLength
                + exifTagsContentLength; // Bytes used for tags contents
    ...
    /**
     * Update the EXIF content referred by a {@link ImageInputStream} using the content available in the
     * {@link ByteArrayOutputStream}. Store the result to the specified {@link OutputStream}.
     *
     * @param outputStream a {@link OutputStream} where to write
     * @param byteStream a {@link ByteArrayOutputStream} previously populated with the content of an EXIF marker.
     * @param inputStream a {@link ImageInputStream} referring to a file containing EXIF metadata to be updated
     * @param originalAPP1MarkerLength is the length of the original APP1 marker (the one containing EXIF content),
     *     before the update
     * @throws IOException
     */
    private static void updateFromStream(
            ...
            final ImageInputStream inputStream,
            final int originalAPP1MarkerLength)
            throws IOException {
    ...
    /**
     * Update the EXIF content of the image stored in the specified byte array, using the content available in the input
     * {@link ByteArrayOutputStream}. Store the result to the specified {@link OutputStream}
     *
     * @param outputStream The stream where to write
    ...
    /**
     * Write the IFDs referred by the tags list argument, to the specified stream. No content referred by a offset, is
     * written.
     *
     * @param stream the {@link ByteArrayOutputStream} where to append the IFDs.
     * @param tags the IFDs to be written. In order to respect the TIFF specification, make sure the element of this
    ...
    /**
     * Write the content of the IFDs referred by the tags list argument, to the specified stream. Note that only the
     * content of the IFDs having an offset will be written.
     *
     * @param stream the {@link ByteArrayOutputStream} where to append the IFDs content.
     * @param tags the Tags list to be written. In order to respect the TIFF specification, make sure the element of
     *     this list are sorted in ascending order.
     * @throws IOException
     */
    ...
    /**
     * Scan the IFDs contained in the input tags collection. Store the offsetValue and size of each entry in the proper
     * array.
     *
     * @param tags the tags to be scan
     * @param baseOffset the baseOffset to be taken into account when computing the tags offsets
    ...
    private static void computeOffsetsAndSizes(
            final List<TIFFTagWrapper> tags, final int baseOffset, final int[] valueOffsets, final int[] sizes) {
     ...
     * @return
     */
    public static final byte[] intToBytes(final int value, final boolean isBigEndian) {
        if (isBigEndian) {
     ...
     * @return
     */
    public static final byte[] intToBytes(final int value) {
     ...
     * @return
     */
    public static final int bytes2ToInt(byte[] buff, int start, final boolean isBigEndian) {
        int intValue = 0;
     ...
     * @return
     */
    public static final int bytes4ToInt(byte[] buff, int start, final boolean isBigEndian) {
        int intValue = 0;
    ...
    /**
     * Return the specified {@link TIFFTagWrapper} as a byte array, by taking endianness into account
     *
     * @param tag
     * @param isBigEndian
     * @return
    ...
    /**
     * Scan the input byte buffer, looking for a candidate bytes sequence and return the index of the first occurrence
     * within the input buffer, or -1 in case nothing is found.
     */
    ...
    /**
     * Scan the input buffer, starting from the specified position, and check whether it matches the candidate byte
     * sequence.
     *
     ...
     */
    static final boolean IsMatch(final byte[] buffer, final int start, final byte[] candidate) {
    ...
    /**
     * Simple utility method which check for conditions which won't allow to get a match from the scan. As an instance,
     * a null candidate byte array or a candidate array longer than the array to be scan won't allow to get a match.
     *
     * @param toBeScan
     * @param candidate
    ...
    /**
     * Replace the EXIF contained within a file referred by a {@link ImageInputStream} instance with the EXIF
     * represented by the specified {@link EXIFMetadata} instance. The original file will be overwritten by the new one
     * containing updated EXIF.
     *
     * <p>It is worth to point out that this replacing method won't currently perform any fields delete, but simply
     * content update. Therefore, tags in the original EXIF which are missing in the updated EXIF parameter, won't be
     * modified.
     *
     ...
     */
    public static void replaceEXIFs(final ImageInputStream inputStream, final EXIFMetadata exif) throws IOException {
    ...
    /**
     * Return an {@link EXIFMetadataWrapper} made of an {@link EXIFMetadata} setup on top of the EXIF found in the
     * inputStream, merged with the EXIF found on the specified exif parameter (if any). The EXIF tags contained in the
     * specified parameter will override the ones found within the inputStream. The returned wrapper will also contain
     * the length of the APP1 marker of the original EXIF.
     *
     ...
     * @return a {@link EXIFMetadataWrapper} containing the merged/updated EXIF as well as the length of the original
     *     APP1 marker
     * @throws IOException
     */
    private static EXIFMetadataWrapper parseExifMetadata(final ImageInputStream inputStream, final EXIFMetadata exif)
            throws IOException {
        ...
        while ((inputStream.read(buff)) != -1) {
            ...
            if (exifTagPos != -1) {
                ...
                final int numBaselineTags = bytes2ToInt(
                        buff, pos + APP1_MARKER.length + 2 + EXIF_MARKER.length + TIFF_HEADER.length, isBigEndian);
                int skip = 0;
    ...
    public static TIFFTagWrapper createTag(int tagNumber) {
        switch (tagNumber) {
            case EXIFTags.USER_COMMENT:
                return new TIFFTagWrapper(
                        EXIFTags.USER_COMMENT,
                        TIFFTag.TIFF_UNDEFINED,
                        null,
                        -1,
                        0,
                        EXIFUtilities.USER_COMMENT_ASCII_CHAR_CODE,
                        null);
            case EXIFTags.COPYRIGHT:
                return new TIFFTagWrapper(
                        EXIFTags.COPYRIGHT, TIFFTag.TIFF_ASCII, null, 0, 0, null, EXIFUtilities.NULL_STRING);
