...
public class NetCDFReaderTest {
    ...
    private static final String filePath = "d:/data/netcdf/";
    private static final String subPath = "/";
    private static final String name = "precip.CRU.DMI.ECC.nc";

    private static final String fileName = filePath + subPath + name;
    ...
    public void testRead() throws IOException {
        ...
        if (numImages / STEP > LIMIT) numImages = LIMIT * STEP;
        ImageIOUtilities.displayImageIOMetadata(
                ncReader.getStreamMetadata().getAsTree(NetCDFStreamMetadata.nativeMetadataFormatName));
        ncReader.dispose();
        for (int i = 0; i < numImages; i += STEP) {
            ncReader.setInput(inputFile);
            final IIOMetadata metadata = ncReader.getImageMetadata(i);
            ImageIOUtilities.displayImageIOMetadata(
                    metadata.getAsTree(CoreCommonImageMetadata.nativeMetadataFormatName));
            ImageIOUtilities.displayImageIOMetadata(metadata.getAsTree(NetCDFImageMetadata.nativeMetadataFormatName));

            if (TestData.isInteractiveTest()) {
                ImageIOUtilities.visualize(ncReader.read(i), "", true);
            } else ncReader.read(i);
            ncReader.dispose();
        }
    }
}
