    ...
    public void testReadSingleFile() throws IOException {
        ...
        File[] files = dir.listFiles(new FileFilter() {
            public boolean accept(File pathname) {
                ...
                if (path.endsWith(".grib") || path.endsWith(".grb")) return true;
                return false;
            }
        });
        for (File inputFile : files) {
            if (LOGGER.isLoggable(Level.INFO)) LOGGER.info("Testing file " + inputFile.getAbsolutePath());
            ...
            try {
                ...
                for (int index = 0; index < nImages; index++) {
                    ...
                    if (TestData.isInteractiveTest()) ImageIOUtilities.visualize(ri);
                    else Assert.assertNotNull(ri.getData());
                    ImageIOUtilities.displayImageIOMetadata(
                            reader.getImageMetadata(index).getAsTree(BaseImageMetadata.nativeMetadataFormatName));
                    ImageIOUtilities.displayImageIOMetadata(
                            reader.getImageMetadata(index).getAsTree(GRIB1ImageMetadata.nativeMetadataFormatName));
                }
            } finally {
                try {
                    reader.dispose();
