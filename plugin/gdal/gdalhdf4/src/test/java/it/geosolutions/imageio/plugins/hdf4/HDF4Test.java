...
public class HDF4Test extends AbstractGDALTest {
    ...
    private static final boolean isLinux =
            ((String) System.getProperty("os.name")).equalsIgnoreCase("Linux") ? true : false;
    /** A simple flag set to true in case the HDF4 driver is available */
    protected static final boolean isDriverAvailable = GDALUtilities.isDriverAvailable("HDF4");

    private static final String msg = "HDF4 Tests are skipped due to missing Driver.\n"
            + "Be sure GDAL has been built against HDF4 and the required"
            + " libs are in the classpath";
    private static final String noSampleDataMsg = new StringBuffer("Test file not available. Please download it at: ")
            .append("http://www.hdfgroup.uiuc.edu/UserSupport/code-")
            .append("examples/sample-programs/convert/Conversion.html")
            .append("\nThen copy it to: plugin/hdf4/src/test/resources/")
            .append("it/geosolutions/imageio/plugins/hdf4/test-data")
            .append(" and repeat the test.")
            .toString();
    ...
    public void subDatasets() throws FileNotFoundException, IOException {
        if (!isLinux && isDriverAvailable) return;
        try {
            ...
            for (int i = startIndex; i < startIndex + loopLength; i++) {
                ...
                // get a RenderedImage
                RenderedOp image =
                        ImageN.create("ImageRead", pbjImageRead, new RenderingHints(ImageN.KEY_IMAGE_LAYOUT, l));
                image.getTiles();
    ...
    public void read() throws FileNotFoundException, IOException {
        if (!isLinux && isDriverAvailable) return;
        try {
            ImageReader mReader = new HDF4ImageReaderSpi().createReaderInstance();
            final String fileName = "TOVS_DAILY_AM_870330_NG.HDF";
            final File file = TestData.file(this, fileName);
            mReader.setInput(file);
            RenderedImage ri = mReader.read(0);
            if (TestData.isInteractiveTest()) Viewer.visualizeAllInformation(ri, "hdf4");
            else {
    ...
    /** This test method retrieves properties from each band of the sample dataset */
    @Test
        ...
        try {
            ...
            for (int i = startIndex; i < startIndex + numImages; i++) {
                ...
                if (it.hasNext()) {
                    ...
                    double d;
                    StringBuffer sb = new StringBuffer("RasterBands properties retrieval")
                            .append(" Image: ")
                            .append(i);
    ...
    public void setUp() throws Exception {
        ...
        ImageN.getDefaultInstance().getTileCache().setMemoryCapacity(180 * 1024 * 1024);
