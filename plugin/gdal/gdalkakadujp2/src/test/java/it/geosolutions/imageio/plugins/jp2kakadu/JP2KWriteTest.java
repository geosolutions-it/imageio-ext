 ...
 *     <p>TODO: we need to rewrite this test using straight image writers. Sugegsted approach: - factor our a method to
 *     centralize writing code, which is repeated over and over (there is an example below) - switch that one method to
 *     use straight writers The issue here is, being able to run this test at all, it requires a Kakadu installation,
 *     and a GDAL build with Kakadu support.
 */
public class JP2KWriteTest extends AbstractJP2KTestCase {
    ...
    // -- sample centralized read/write methods for reuse in tests --
    ...
    //    /**
    //     * Writes a JP2 file applying test-specific configuration via a lambda.
    //     * @param image the source image to write
    //     * @param outName the output filename (will be created via TestData.temp)
    //     * @param config a lambda that mutates the JP2 write params for this test
    //     * @return the created output file
    //     */
    //    private File writeJP2(RenderedOp image, String outName,
    //                          java.util.function.Consumer<JP2GDALKakaduImageWriteParam> config)
    //            throws IOException {
    //
    ...
    //        JP2GDALKakaduImageWriteParam jp2 = (JP2GDALKakaduImageWriteParam) iwp;
    //
    //        // Let the test set its specifics
    //        if (config != null) {
    //            config.accept(jp2);
    //        }
    //
