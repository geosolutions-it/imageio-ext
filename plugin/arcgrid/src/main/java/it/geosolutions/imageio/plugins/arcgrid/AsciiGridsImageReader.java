...
/**
 * Class used for reading ASCII ArcInfo Grid Format (ArcGrid) and ASCII GRASS Grid Format and to create
 * {@link RenderedImage}s and {@link Raster}s.
 *
...
public final class AsciiGridsImageReader extends ImageReader {
    ...
    /**
     * The thread-safe {@link AsciiGridRaster} to read rasters out of an ascii grid file.
     *
     * <p>Every {@link AsciiGridsImageReader} will cache this raster-reader between differet reads because it will
     * internally save information about the positions of the tiles on disk.
     */
     ...
     *
     * <p>The color model is always the same, moreover a {@link ColorModel} in java is an immutable, therefore it is
     * possible to create it just once for all the possible {@link AsciiGridsImageReader}.
     */
    private static final ComponentColorModel cm = RasterFactory.createComponentColorModel(
     ...
     * Constructor.
     *
     * <p>It builts up an {@link AsciiGridsImageReader} by providing an {@link AsciiGridsImageReaderSpi}
     *
     ...
     *
     * @param input Source the {@link AsciiGridsImageReader} will read from
     *     <p><strong>NOTE: Constrain on GZipped InputStream</strong> If we want to provide explicitly an InputStream
     *     (instead of a File) for a GZipped source, we MUST provide a proper previously created GZIPInputStream instead
     *     of a simple InputStream.
     *     <p>Thus, you need to use Code A) instead of Code B):
     *     <blockquote>
     *     //as an instance: File file = new File("example.asc.gz"); //A GZipped Source ... //Code A) GZIPInputStream
     *     stream = new GZIPInputStream(new FileInputStream(file));
     *     <p>//Code B) //InputStream stream = new FileInputStream(file);
     *     </blockquote>
     *     Otherwise, when calling {@code ImageIO.getImageReaders(stream)}, (directly or indirectly by a Jai ImageRead
     *     Operation), the proper SPI can't correctly try to read the Header in order to decode the input.
     */
    public void setInput(Object input) {
        ...
        if (this.imageInputStream != null) {
            reset();
            if (LOGGER.isLoggable(Level.FINE)) LOGGER.fine("Resetting old stream");
        }
        ...
        //
        // ////////////////////////////////////////////////////////////////////
        if (input instanceof ImageInputStream) imageInputStream = (ImageInputStream) input;
        else
        ...
        if (input instanceof URL) {
            ...
            if (!testUrl.getProtocol().equalsIgnoreCase("file")) {
                ...
                if (LOGGER.isLoggable(Level.SEVERE)) LOGGER.severe("Unsupported URL provided as input!");
            ...
            if (!inFile.exists()) {
                // is not a file let's reject it
                if (LOGGER.isLoggable(Level.SEVERE)) LOGGER.severe("Input file does not exists!");
        ...
        if (input instanceof File) {
            ...
            if (LOGGER.isLoggable(Level.SEVERE)) LOGGER.severe("Input is not decodable!");
        ...
        try {
            ...
            final IllegalArgumentException ex =
                    new IllegalArgumentException("Unable to parse the header for the provided input");
            ex.initCause(e);
            throw ex;
        }
        ...
        try {
            ...
            try {
                ...
                if (LOGGER.isLoggable(Level.SEVERE)) LOGGER.log(Level.SEVERE, e1.getLocalizedMessage(), e1);
                final IllegalArgumentException ex =
                        new IllegalArgumentException("Unable to parse the header for the provided input");
                ex.initCause(e1);
                throw ex;
    ...
    /**
     * This method initializes the {@link AsciiGridsImageReader} (if it has already decoded an input source) by setting
     * some fields, like the imageInputStream, the {@link ColorModel} and the {@link SampleModel}, the image dimensions
     * and so on.
     */
    ...
    /**
     * This method check if the ImageIndex indicated is valid. For {@link AsciiGridsImageReader} the only legal
     * imageIndex value is 0, since AsciiGrid sources may contain data for just a single image
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
     * @throws IndexOutOfBoundsException if imageIndex is greater than 0.
     */
    private void checkImageIndex(final int imageIndex) {
        /* AsciiGrid file format can "contain" only 1 image */
        if (imageIndex != 0) throw new IndexOutOfBoundsException("illegal Index: " + imageIndex);
    }
    ...
    /**
     * Returns the number of images available from the current input source. Since AsciiGrid input source only contains
     * data for a single image, this method always return "1". The input parameter is simply ignored.
     *
     * @return 1
     */
    ...
    /**
     * this method provides suggestions for possible image types that will be used to decode the image. In this case, we
     * are suggesting using a 32 bit grayscale image with no alpha component.
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
     * @return an <code>Iterator</code> containing an <code>ImageTypeSpecifier</code> suggesting to use a 32 bit
     *     grayscale image.
     */
    public synchronized Iterator<ImageTypeSpecifier> getImageTypes(final int imageIndex) throws IOException {
        ...
        if (imageType == null) imageType = new ImageTypeSpecifier(cm, sm);
        l.add(imageType);
        return l.iterator();
    }

    /**
     * Since Ascii Grid format sources may only contain data for a single image, we return <code>null</code>. We suggest
     * to use <code>getImageMetadata(0)</code> in order to retrieve valid metadata
     *
    ...
    /**
     * Returns an <code>IIOMetadata</code> object containing metadata associated with the image.
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
    ...
    public IIOMetadata getImageMetadata(final int imageIndex) throws IOException {
        checkImageIndex(imageIndex);
        if (metadata == null) metadata = new AsciiGridsImageMetadata(this.rasterReader);
        return metadata;
    }

    /**
     * Reads the raster and return it as a complete <code>BufferedImage</code> using a supplied <code>ImageReadParam
     * </code>
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
     * @param param an <code>ImageReadParam</code> to specify subsampling factors, and sourceRegion settings. Other
     *     properties are actually ignored.
    ...
    public BufferedImage read(final int imageIndex, ImageReadParam param) throws IOException {
        if (LOGGER.isLoggable(Level.FINE)) LOGGER.info("read(final int imageIndex, ImageReadParam param)");
        ...
        final BufferedImage bi = new BufferedImage(cm, (WritableRaster) readRaster(imageIndex, param), false, null);

        if (hasListeners) {
            // Check if there is a request of aborting the read and broadcast
            // the proper event.
            if (rasterReader.isAborting()) processReadAborted();
    ...
    /**
     * Returns <code>true</code> if the image has been tiled. All AsciiGrid sources are untiled. However, when the size
     * of an image is greater than a threshold value, we introduce a tiling mechanism.
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
    ...
    /**
     * Returns <code>true</code> if the storage format of the image places no inherent impediment on random access to
     * pixels. Since each value contained within an AsciiGrid file may be represented with a different number of decimal
     * digits, we need to find/count a specific number of whitespaces before to get a desired pixel value. For this
     * reason, this method returns <code>false</code>
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
    ...
    /**
     * Returns <code>true</code> since we always call the <code>setInput</code> method with the <code>seekForwardOnly
    ...
    /**
     * Simply call the overloaded <code>read</code> method by passing <code>null</code> as value of the <code>
     * ImageReadParam</code> argument.
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
    ...
    public BufferedImage read(final int imageIndex) throws IOException {
        checkImageIndex(imageIndex);
        if (LOGGER.isLoggable(Level.FINE)) LOGGER.info("read(imageIndex");
        return read(imageIndex, null);
    }

    /**
     * Returns <code>false</code> since AsciiGrid format does not supports thumbnail preview images.
     *
     * @return <code>false</code>
     */
    ...
    /**
     * Returns a new <code>Raster</code> object containing the raw pixel data from the image stream, without any color
     * conversion applied.
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
    ...
    public Raster readRaster(final int imageIndex, ImageReadParam param) throws IOException {
        if (LOGGER.isLoggable(Level.FINE)) LOGGER.info("readRaster(final int imageIndex, ImageReadParam param)");
        if (param == null) param = getDefaultReadParam();
        return rasterReader.readRaster(param);
    }

    /**
     * Reads the tile indicated by the <code>tileX</code> and <code>tileY</code> arguments, returning it as a <code>
     * BufferedImage</code>.
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
    ...
    /**
     * Returns a new <code>Raster</code> object containing the raw pixel data from the tile, without any color
     * conversion applied.
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
    ...
    public Raster readTileRaster(final int imageIndex, int tileX, int tileY) throws IOException {
        if (LOGGER.isLoggable(Level.FINE)) LOGGER.info("readTileRaster(final int imageIndex, int tileX, int tileY)");
        return readTile(imageIndex, tileX, tileY).getRaster();
    }

    /**
     * Returns <code>true</code> since this plug-in supports reading just a {@link java.awt.image.Raster
     * <code>Raster</code>} of pixel data.
     *
     * @return <code>true</code>
     */
    ...
    /**
     * Returns the smallest valid index for reading, 0 for the {@link AsciiGridsImageReader}.
     *
     * @return 0
     */
    ...
    /**
     * Returns the number of thumbnail preview images associated with the given image. Since
     * {@link AsciiGridsImageReader} does not support thumbnails, this method always returns 0.
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
     * @return 0
    ...
    /**
     * Always return false since the {@link AsciiGridsImageReader} does not support thumbnails
     *
     * @param imageIndex the index of the required image which need to be always 0 since AsciiGrid format supports only
     *     single image.
     * @return <code>false</code>
    ...
    /**
     * Returns <code>true</code> if the current input source has been marked as allowing metadata to be ignored by
     * passing <code>true</code> as the <code>ignoreMetadata</code> argument to the
    ...
    public synchronized void abort() {
        // super.abort();
        if (rasterReader != null) rasterReader.abort();
    }
    ...
    protected synchronized void clearAbortRequest() {
        ...
        if (rasterReader != null) rasterReader.clearAbort();
    }

    /** A Simple call to the <code>setInput(Object input)</code> method, ignoring all other parameters. */
