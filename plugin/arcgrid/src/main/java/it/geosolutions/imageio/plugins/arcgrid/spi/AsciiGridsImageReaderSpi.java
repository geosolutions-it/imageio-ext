...
/**
 * Class which provides a specialized Service Provider Interface which instantiates an AsciiGridsImageReader if it is
 * able to decode the input provided.
 *
...
public final class AsciiGridsImageReaderSpi extends ImageReaderSpi {
    ...
    static final String nativeImageMetadataFormatName =
            "org.geotools.gce.imageio.asciigrid.AsciiGridsImageMetadata_1.0";

    static final String nativeImageMetadataFormatClassName =
            "org.geotools.gce.imageio.asciigrid.AsciiGridsImageMetadataFormat";
    ...
    public AsciiGridsImageReaderSpi() {
        super(
                ...
                new Class[] {File.class, ImageInputStream.class},
        ...
        if (LOGGER.isLoggable(Level.FINE)) LOGGER.info("Creating a SPI");
    }

    /**
     * This method check if the input source can be decoded by the reader provided by this specific subclass of
     * ImageReaderSpi. Return true if the check was successfully passed input source type accepted and handled are
    ...
    public boolean canDecodeInput(Object input) throws IOException {
        if (LOGGER.isLoggable(Level.FINE)) LOGGER.info("canDecodeInput?");

        if (input instanceof ImageInputStream) ((ImageInputStream) input).mark();
        ...
        // it before leaving
        /** Checking input source types and creating an ImageInputStream from them, needed for header parsing */
        ...
        if (input instanceof URL) {
            ...
            if (tempURL.getProtocol().equalsIgnoreCase("file")) input = ImageIOUtilities.urlToFile(tempURL);
            else input = ((URL) input).openStream();
            closeMe = true;
        }
        ...
        if (input instanceof InputStream) {
             ...
             *
             * <p>Example A) final File f = new File("file.asc.gz"); final InputStream is = new FileInputStream(f);
             * Iterator it = ImageIO.getImageReaders(stream);
             *
             * <p>Example B) final File f = new File("file.asc.gz"); final GZIPInputStream stream = new GZIPInputStream(
             * new FileInputStream(f)); Iterator it = ImageIO.getImageReaders(stream);
             *
             * <p>if we use the code of the Example A instead of that of Example B, the parseHeader method (which works
             * with ImageInputStreams) is unable to try to read the header because the stream is GZipped.
             *
             * <p>Thus, if we want to provide explicitly an InputStream (instead of a File) for a GZipped File, we need
             * to use the code of the Example B which make use of the GZIPInputStream Subclass.
             */
        ...
        try {
            ...
            try {
                ...
                if (closeMe) spiImageInputStream.close();
                return false;
            }
        }
        // /////////////////////////////////////////////////////////////////////
        ...
        // /////////////////////////////////////////////////////////////////////
        if (input instanceof ImageInputStream) ((ImageInputStream) input).reset();
        if (closeMe) spiImageInputStream.close();
        return true;
    }
