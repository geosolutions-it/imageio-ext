...
public abstract class AsciiGridRaster {
    ...
    public static enum AsciiGridRasterType {
        ...
        ESRI {
            @Override
            public EsriAsciiGridRaster createAsciiGridRaster(final ImageInputStream inStream) {
    ...
    /** x coordinate of the grid origin (the lower left corner) in compliance with the <code>isCorner</code> value */
    protected double xllCellCoordinate = Double.NaN;

    /** y coordinate of the grid origin (the lower left corner) in compliance with the <code>isCorner</code> value */
    ...
    /**
     * The grid's origin (the lower left corner of the grid) may be specified using the coordinates of either its
     * lower-left corner (by providing XLLCORNER and YLLCORNER) or the center of the lower-left grid cell (XLLCENTER and
     * YLLCENTER)
     */
    /**
     * If <code>isCorner</code> is <code>true</code> then <code>xllCellCoordinate</code> is the coordinate of the
     * lower-left corner of the grid. If <code>isCorner</code> is <code>false</code> then <code>xllCellCoordinate</code>
     * is the coordinate of the center of the lower-left gridcell of the grid.
     */
    ...
    /**
     * Used to remember the locations (in bytes offset from 0) of samples at tile boundaries within the data. These
     * locations speed up random access to tile data by allowing us to seek to the closest-lower known position in the
     * file to start reading from there. Without this, due to the irregular layout (on disk) of the data, random tile
     * access would require a large amount of data to be needlessly skipped through before the right place in the file
     * can be found. Example: If we had ascii data that was laid out in the stream like so...
     *
     * <pre>
     * 0          1          2          3          4          5
    ...
    /**
     * A constructor to build an {@link AsciiGridRaster} given an <code>ImageInputStream</code> and an
     * {@link AsciiGridsImageReader}
     *
     * @param iis <code>ImageInputStream</code> needed to read the raster.
     * @param reader {@link AsciiGridsImageReader} used to read the raster.
     */
     ...
     *
     * @param ios <code>ImageOutputStream</code> needed to write the raster.
     */
    ...
    /**
     * A constructor to build an {@link AsciiGridRaster} given an <code>ImageOutputStream</code> and an
     * {@link AsciiGridsImageWriter}
     *
     * @param iis <code>ImageOutputStream</code> needed to read the raster.
     * @param writer {@link AsciiGridsImageWriter} used to read the raster.
     */
    ...
    /**
     * The header structure of an ASCII GRASS file is different from the one of an ASCII ArcGrid file.<br>
     *
     * <pre>
    ...
    /**
     * As stated for {@link parseHeader}, the differences betweens the structure of the header of an ASCII ArcGrid and
     * the one of an ASCII GRASS require a different management also during the writing process. For this reason, a
     ...
     * GRASS use the '*' sign to represent noData, while ArcGrid use a numeric representation (A commonly used value is
     * -9999 which is also used as default value)
     */
     ...
     */
    public final void setTilesSize(final int tileWidth, final int tileHeight) {
     ...
     * It is <code>true</code> if <code>xllCellCoordinate</code> and <code>yllCellCoordinate</code> represent the
     * coordinates of the lower-left corner of the lower-left cell of the grid. It is <code>false</code> if they
     * represent the coordinates of the center of the lower-left cell of the grid.
     *
     * @return isCorner.
     */
    ...
    /**
     * Returns the stream position where useful data starts (just after the end of the header)
     *
     * @return dataStartAt
     */
     ...
     * When image is tiled or reading is executed only on a specific part of the ASCII source, I need to determine which
     * values must be loaded and which must be skipped. Within an ASCII source, I can't know how many digits compose a
     * value. Thus, I need to scan and check every byte stored on the input source and retrieve the value as well as I
     * need to skip values if they are useless.
     *
     * @param param an ImageReadParam which specifies source region properties as width, height, x and y offsets.
     * @return WritableRaster the Raster composed by reading data values
     * @throws IOException
     * @throws NumberFormatException TODO we ignore destination region, destinationOffset etc...
     */
    public WritableRaster readRaster(ImageReadParam param) throws IOException {
        ...
        if (srcRegion != null) {
            ...
            //
            // //
            if (srcRegionXOffset < 0) srcRegionXOffset = 0;
            if (srcRegionYOffset < 0) srcRegionYOffset = 0;
        ...
        if (xSubsamplingFactor > 1 || ySubsamplingFactor > 1) doSubsampling = true;
        // ////////////////////////////////////////////////////////////////////////////
        //
        ...
        if (factory != null)
            raster = factory.createTile(
                    RasterFactory.createBandedSampleModel(
                            java.awt.image.DataBuffer.TYPE_DOUBLE, dstWidth, dstHeight, 1),
                    null);
        else
            raster = RasterFactory.createBandedRaster(
                    java.awt.image.DataBuffer.TYPE_DOUBLE, dstWidth, dstHeight, 1, null);
                ...
                if (exactPosition != null) {
                    if (LOGGER.getLevel() == Level.FINEST) {
                        LOGGER.finest(String.format(
                                ...
                                exactPosition, tileBeginsAtSampleIndex));
                    }
                    // this is an exact match - no sample skipping will be required, just set the stream position and we
                    // can start
                    ...
                    final SortedMap<Long, Long> partitionedIndex =
                            tileMarker.headMap(Long.valueOf(tileBeginsAtSampleIndex));

                    if (!partitionedIndex.entrySet().isEmpty()) {
                        // the highest/last key in the partitioned index will give us the best possible position to
                        // start from
                        // as it will be the closest to where we need to start reading from (given we read from left to
                        // right)
                        ...
                        if (LOGGER.getLevel() == Level.FINEST) {
                            LOGGER.finest(String.format(
                                    "[lookup] Found nearest (%d) indexed stream position %d for sample %d%n",
                                    tileBeginsAtSampleIndex, nearestPosition, nearestSampleIndex));
                        }

                        indexedSeekPos = nearestPosition;
                        nextSampleIndex = nearestSampleIndex;
                    } else {
                        // NB pretty sure this can't ever happen, given that the index is populated from l->r, but let's
                        // leave it
                        // here as the test coverage doesn't give me much confidence that it's not needed for obscure
                        // reasons
        ...
        if (LOGGER.getLevel() == Level.FINEST) {
            LOGGER.finest(String.format(
                    "[lookup] After index lookup : indexedSeekPos=%d nextSampleIndex=%d%n",
        ...
        if (samplesToSkip > 0) {
            ...
            if (LOGGER.getLevel() == Level.FINEST) {
                LOGGER.finest(String.format(
                        "[skipping] About to skip %d samples to go from %d to %d%n",
            ...
            while (numSamplesSkipped < samplesToSkip) {
                ...
                if (ch == -1) throw new EOFException("EOF found while looking for valid input");
                streamPosition++;
                        ...
                        if (((nextSampleIndex) % (tileH * tileW)) == 0) {
                            ...
                            synchronized (tileTreeMutex) {
                                if (!tileMarker.containsKey(key)) {
                                    if (LOGGER.getLevel() == Level.FINEST) {
                                        LOGGER.finest(String.format(
                                                "[skipping] Reached tile boundary, marking sample position %d will be found at stream position %d%n",
                                                key, val));
                                    }
                ...
                if (hasListeners) {
                    ...
                    if ((perc % (10 * iPerc) == 0) && (int) perc > 0) {
                        if (abortRequired) return raster;
                        iPerc++;
                    }
                }
            }
        ...
        if (numSamplesSkipped > 0) {
            if (LOGGER.getLevel() == Level.FINEST) {
                LOGGER.finest(String.format(
                        "[skipping] Finished, numSamplesSkipped=%d, nextSampleIndex=%d streamPosition=%d%n",
            ...
            if (LOGGER.getLevel() == Level.FINEST) {
                LOGGER.finest(String.format(
                        "[skipping] No skipping required, nextSampleIndex=%d streamPosition=%d%n",
        ...
        if (LOGGER.getLevel() == Level.FINEST) {
            LOGGER.finest(String.format(
                    "[reading] starting read of %d samples %d..%d stream position %d%n",
        ...
        while (samplesRead < samplesToLoad) {
            ...
            if (!((tempCol < srcRegionXOffset || tempCol >= srcRegionXOffset + srcRegionWidth))) {

                if ((!doSubsampling)
                        || (doSubsampling
                                && (((tempRow) % ySubsamplingFactor == 0) && ((tempCol) % xSubsamplingFactor == 0)))) {
        ...
        synchronized (tileTreeMutex) {
            ...
            if (LOGGER.getLevel() == Level.FINEST) {
                LOGGER.finest(String.format(
                        "[reading] Finished reading, marking sample position %d will begin at stream position %d%n",
                        samplePositionToCache, streamPositionToCache));
            }
     ...
     * Writes the raster
     *
     * @param iterator A <code>RectIterator</code> built on Lines and Pixels of the Raster which need to be written.
     * @param noDataDouble the value representing noData.
     * @param noDataMarker a <code>String</code> which need to be printed when founding a noData value
     * @throws IOException
     */
    public void writeRaster(RectIter iterator, Double noDataDouble, String noDataMarker) throws IOException {
        ...
        final int pixelsToWrite = hasListeners ? (writer.getNColumns() * writer.getNRows()) : 0;
        int pixelsWritten = 0;
        int perc = 0;
        int iPerc = 1;
        ...
        while (!iterator.finishedLines()) {
            while (!iterator.finishedPixels()) {
                ...
                // space
                if (!iterator.finishedPixels()) imageOS.write(32);
            }
    ...
    /**
     * Retrieves a value from the ascii grid header using the provided {@link ImageInputStream} and by reading at most
     * <code>maxBytesToRead</code> from the provided input stream.
     *
     * <p>The maximum length of the value we are looking for is <code>maxValueLength</code>.
     *
     * <p>The header of an ascii grid file, ESRI or GRASS is composed by a certain number of lines of the form:
     *
     * <p>key value _new_line_ key value _new_line_
     *
     * <p>Purpose of this method is to look for the header's keys.
     *
     * @param inStream is the {@link ImageInputStream} to read from.
     * @param maxBytesToRead is maximum number of bytes to consume from the provided {@link ImageInputStream}.
     * @param maxValueLength indicates the maximum expected length in input ascii characters of the value we are looking
     *     for.
     * @return a {@link double} containing either a valid value or Negative Infinity if something bad happened.
     * @throws IOException
     */
    double getValue(
            final ImageInputStream inStream, int maxBytesToRead, int maxValueLength, StringToDouble doubleConverter)
            throws IOException {
        ...
        while (true) {
            ...
            //
            // ///////////////////////////////////////////////////////////////////
            if (!doubleConverter.pushChar(b)) return Double.NEGATIVE_INFINITY;
            started = true;
    ...
    /**
     * Retrieves a key from the ascii grid header using the provided {@link ImageInputStream} and by reading at most
     * <code>maxBytesToRead</code> from the provided input stream.
     *
     * <p>The maximum length of the key we are looking for is <code>maxKeyLength</code>.
     *
     * <p>The <code>specialChar</code> input params can be used to recognize special input character beyond the usual
     * ones [A-Z], [a-z], _.
     *
     * <p>The header of an ascii grid file, ESRI or GRASS is composed by a certain number of lines of the form:
     *
     ...
     * @param inStream is the {@link ImageInputStream} to read from.
     * @param maxBytesToRead is maximum number of bytes to consume from the provided {@link ImageInputStream}.
     * @param maxKeyLength indicates the maximum expected length of they key we are looking for.
     * @param specialChar can be used to indicate a special character to decode (see above).
     * @return a {@link String} containing either a key or an empty string.
     * @throws IOException
     */
    String getKey(final ImageInputStream inStream, int maxBytesToRead, int maxKeyLength, byte specialChar)
            throws IOException {
        ...
        while (true) {
            ...
            // //
            if (specialChar > 127) {
                if (!(b == 95 || (b >= 97 && b <= 122) || (b >= 64 && b <= 90))) break;
            } else if (!(b == 95 || b == specialChar || (b >= 97 && b <= 122) || (b >= 64 && b <= 90))) break;
            ...
            started = true;
            if (validBytesRead >= maxKeyLength) break;
        }
