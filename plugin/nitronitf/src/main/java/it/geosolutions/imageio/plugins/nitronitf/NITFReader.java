...
public class NITFReader extends ImageReader {
    ...
    private Map<Integer, nitf.ImageReader> imageReaderMap =
            Collections.synchronizedMap(new HashMap<Integer, nitf.ImageReader>());
    ...
    private synchronized nitf.ImageReader getImageReader(int imageIndex) throws IOException {
        ...
        try {
            if (!imageReaderMap.containsKey(key)) imageReaderMap.put(key, reader.getNewImageReader(imageIndex));
    ...
    /**
     * Optimization to read the entire image in one fell swoop... This is most likely the common use case for this
     * codec, so we hope this optimization will be helpful.
     *
     * @param imageIndex
            ...
            if (bandOffsets.length == 1) {
                ...
                switch (pixelSize) {
                    case 1:
                        ...
                        break;
                    case 2:
                        ShortBuffer rasterShortBuf =
                                ShortBuffer.wrap(((DataBufferUShort) imRas.getDataBuffer()).getData());
                        rasterShortBuf.put(bandBuf.asShortBuffer());
                        break;
                    case 4:
                        FloatBuffer rasterFloatBuf =
                                FloatBuffer.wrap(((DataBufferFloat) imRas.getDataBuffer()).getData());
                        rasterFloatBuf.put(bandBuf.asFloatBuffer());
                        break;
                    case 8:
                        DoubleBuffer rasterDoubleBuf =
                                DoubleBuffer.wrap(((DataBufferDouble) imRas.getDataBuffer()).getData());
    ...
    protected void readRaster(
        ...
        try {
            ...
            if ((destRegion.height * sourceYSubsampling) == numRows
                ...
                readFullImage(
                        imageIndex, destRegion, sourceXSubsampling, sourceYSubsampling, bandOffsets, pixelSize, imRas);
                return;
            }
    ...
    public BufferedImage read(int imageIndex, ImageReadParam param) throws IOException {
        ...
        int numDestBands = param != null
                ? (param.getDestinationBands() != null
                        ? param.getDestinationBands().length
                        : param.getSourceBands() != null ? param.getSourceBands().length : 0)
                : 0;
        ...
        while (imageTypes.hasNext() && imageType == null) {
            ...
            if (firstType == null) firstType = currentImageType;

            if (currentImageType.getNumBands() == numDestBands) imageType = currentImageType;
        }

        if (imageType == null) {
            if (firstType == null) throw new IOException("Unable to determine the ImageTypeSpecifier");
            else imageType = firstType;
        }
