...
public class GeoTiffCreateOptionsHandler extends GDALCreateOptionsHandler {
		 ...
		 * (.tfw).See a World Files section for details..<BR />
		 * 
		 ...
		 * 
		 * TILED=YES: By default stripped TIFF files are created. This option
		 * can be used to force creation of tiled TIFF files.<BR />
		 * 
		 ...
		 * 
		 * COMPRESS=[JPEG/LZW/PACKBITS/DEFLATE/NONE]: Set the compression to
		 * use. JPEG should only be used with Byte data. None is the default..<BR />
		 * 
		 * PREDICTOR=[1/2/3]: Set the predictor for LZW or DEFLATE compression.
		 * The default is 1 (no predictor), 2 is horizontal differencing and 3
		 * is floating point prediction..<BR />
		 * 
		 * JPEG_QUALITY=[1-100]: Set the JPEG quality when using JPEG
		 * compression. A value of 100 is best quality (least compression), and
		 * 1 is worst quality (best compression). The default is 75.<BR />
		 * 
		 * PROFILE=[GDALGeoTIFF/GeoTIFF/BASELINE]: Control what non-baseline
		 * tags are emitted by GDAL. With GDALGeoTIFF (the default) various GDAL
		 * custom tags may be written. With GeoTIFF only GeoTIFF tags will be
		 * added to the baseline. With BASELINE no GDAL or GeoTIFF tags will be
		 * written. BASELINE is occationally useful when writing files to be
		 * read by applications intolerant of unrecognised tags.<BR />
		 * 
		 ...
		 * selected. You can override default photometric using this option.<BR />
		 * 
		 * ALPHA=YES: The first "extrasample" is marked as being alpha if there
		 * are any extra samples. This is necessary if you want to produce a
		 * greyscale TIFF file with an alpha band (for instance).<BR />
		 */
